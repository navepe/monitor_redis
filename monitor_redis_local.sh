...
if [[ $hostname == "redis-001" ]]; then   ### if hostname is redis-001
	for port in "${ports[@]}"; do    ### run on ports 7000, 7001 & 7002
		...
		if [[ $current_connection_port -gt $limit_port ]]; then  ### if the current is bigger than 80%
			message_port="$HOSTNAME"' - on '"$port"' - Too Many Connections in redis' ### create a msg
			if [[ " ${nbd_env[*]} " =~ "redis-001" ]]; then   ### if the instance is member of nbd env - send a msg in the channel
				python /home/nave/work/repos/scripts/send_message_to_slack.py "$message_port" "nbd_alerts_channel"
			elif [[ " ${sparta_env[*]} " =~ "redis-001" ]]; then   ### sparta env
				python /home/nave/work/repos/scripts/send_message_to_slack.py "$message_port" "sparta_alerts_channel"
			else
				python /home/nave/work/repos/scripts/send_message_to_slack.py "$message_port" "devops_alerts_channel" ### the rest send to devops
			fi
		fi

		if ! [[ -n $current_connectionn_port || $limit_port ]]; then  ### if there is no value in current or limit - too many connection and it failes
			connection_failed_port="$HOSTNAME"' - on '"$port"' - Failed to connect to redis'
		        if [[ " ${nbd_env[*]} " =~ "redis-001" ]]; then
			        python /home/nave/work/repos/scripts/send_message_to_slack.py "$connection_failed_port" "nbd_alerts_channel"
		        elif [[ " ${sparta_env[*]} " =~ "redis-001" ]]; then
			        python /home/nave/work/repos/scripts/send_message_to_slack.py "$connection_failed_port" "sparta_alerts_channel"
		        else
			        python /home/nave/work/repos/scripts/send_message_to_slack.py "$connection_failed_port" "devops_alerts_channel"
			fi
		fi
